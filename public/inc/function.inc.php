...
function select($sql) {
  ...
  $result = array();
  $req = $conn->query($sql);
  //$req = mysqli_query($conn, $sql) or die("SQL Error: " . $sql . "" . mysqli_connect_errno());
...
function insert($table, $data) {
  ...
  if (mysqli_query($conn, $sql)) {
    ...
    print("SQL Error: " . $sql . "" . mysqli_connect_error());
    debug_to_console("SQL Error: " . $sql . "" . mysqli_connect_errno());
    return false;
  }
//    //Example of use:
//$data = array(
...
function debug_to_console($data) {
  global $conn;
    ...
    $output = "<script>console.log( 'Debug Objects: " . mysqli_real_escape_string($conn, $data) . "' ); </script>";
    echo $output;
}
...
function getDepartment_v2($dbname, $tbl_department, $tbl_document, $dep_id, $dep_to = '') {
    // แสดงชื่อแผนกที่เคยส่งเอกสารกับหน่วยงาน by Sirivimol T. 14-AUG-2013
    $arr_return = "";
    $str_add = '';
    if ($dep_id != "") {
...
function getEmployee($tbl_employee, $price_status = '', $emp_search_name = "", $dep_search_id = '', $emp_id_search = '', $register = 'register', $give_up = 'N') {
    ...
    if ($emp_search_name != "") {
        $str_add .= " (d.`emp_firstname` like '%" . mysql_real_escape_string($emp_search_name) . "%' or d.`emp_surname` like '%" . mysql_real_escape_string($emp_search_name) . "%') and";
    } // end if
    if ($emp_id_search != "") {
        $str_add .= " d.`emp_id` like '%" . mysql_real_escape_string($emp_id_search) . "%' and";
    } // end if
    if ($dep_search_id != "") {
        $str_add .= " d.`dep_id` = '" . mysql_real_escape_string($dep_search_id) . "' and";
    } // end if
    if ($register != "") {
        if ($register == 'none') {
            $str_add .= " (d.`register` = 'N' and d.`register2` = 'N') and ";
        } else {
            $str_add .= " d.`" . $register . "` = 'Y' and";
        }
    } // end if
    if ($give_up != "") {
        $str_add .= " d.`give_up` = '" . mysql_real_escape_string($give_up) . "' and";
    } // end if
    ...
    $sql = "SELECT  d.* FROM $tbl_employee as d  $str_add  order by d.register, d.register2, d.emp_id,d.dep_name, d.prize_no, d.emp_firstname, d.emp_surname ";
    //    echo $sql . "<br>";
    $result = mysql_query($sql);
    //   echo $result . "<br>";
    return $result;
...
//        echo ' cnt = ' . $count . "<br>";
    ...
    if ($count == 0) {
        // if no data
        $sql = 'SELECT * FROM ' . TBL_EMPLOYEE . ' where give_up = \'N\' and register=\'Y\' ORDER BY RAND() LIMIT ' . $limit;
        $result = select($sql);
        if ($show_console) {
            echo "<pre>";
            print_r($result);
...
function showLucky($max_row = 5) {
    ...
    if ($result_show != '') {
        foreach ($result_show as $line => $row) {
            ...
            $dep_name = $row["dep_name"];
            $str .= '<tr><td class="tdlucky">' . ($line + 1) . '.</td><td class="tdlucky">' . $prefix . " " . $firstname . " " . $surname . ' &nbsp;&nbsp;(' . $emp_id . ') <br><span class="tdlucky2">' . $dep_name . "</span></td></tr>";
            if ((($line + 1) % $max_row) == 0) {
                $status_show = true;
            } // end if
